...
def train():
    ...
    for epoch in trange(args.epochs, desc='Training_epoch'):
        ...
        for idx, (data, label) in enumerate(train_loader):
            ...
            ssl_output, ssl_output2, ssl_output3, cls_output = model(accel_x, gyro_x)
            ssl_label2 = torch.arange(ssl_output2.size(0)).to(device)
            ssl_label3 = torch.arange(ssl_output3.size(0)).to(device)
            ssl_label = torch.arange(ssl_output.size(0)).to(device)

            ssl_loss2 = (criterion_cls(ssl_output2, ssl_label2) + criterion_cls(ssl_output2.T, ssl_label2)) / 2
            ssl_loss3 = (criterion_cls(ssl_output3, ssl_label3) + criterion_cls(ssl_output3.T, ssl_label3)) / 2
            ...
            loss = ssl_loss * args.lambda_ssl + ssl_loss2 * args.lambda_ssl2 + ssl_loss3 * args.lambda_ssl3 + cls_loss * args.lambda_cls
            ...
            _, ssl_predicted2 = torch.max(ssl_output2, 1)
            _, ssl_predicted3 = torch.max(ssl_output3, 1)
            _, ssl_predicted = torch.max(ssl_output, 1)
            _, cls_predicted = torch.max(cls_output, 1)
            ssl_gt2.append(ssl_label2)
            ssl_pred2.append(ssl_predicted2)
            ssl_gt3.append(ssl_label3)
            ssl_pred3.append(ssl_predicted3)
        ...
        label2_2 = torch.cat(ssl_gt2).cpu().detach().numpy()
        predicted2_2 = torch.cat(ssl_pred2).cpu().detach().numpy()
        acc_train2_2 = (predicted2_2 == label2_2).sum() * 100.0 / len(label2_2)
        label2_3 = torch.cat(ssl_gt3).cpu().detach().numpy()
        predicted2_3 = torch.cat(ssl_pred3).cpu().detach().numpy()
        acc_train2_3 = (predicted2_3 == label2_3).sum() * 100.0 / len(label2_3)
        ...
        logger.info('Epoch: [{}/{}] - loss:{:.4f}, train acc: {:.2f}%, train F1: {:.4f} | ssl acc: {:.2f}%, ssl2 acc: {:.2f}%, ssl3 acc: {:.2f}%'.format(epoch, args.epochs, total_loss / len(train_set), acc_train, f1_train, acc_train2, acc_train2_2, acc_train2_3))

        writer.add_scalar('Train/Accuracy_cls', acc_train, epoch)
        writer.add_scalar('Train/F1_cls', f1_train, epoch)
        writer.add_scalar('Train/Accuracy_ssl2', acc_train2_2, epoch)
        writer.add_scalar('Train/Accuracy_ssl3', acc_train2_3, epoch)
...
def evaluate(model, eval_loader, epoch, is_test=True, mode='best'):
    ...
    with torch.no_grad():
        for idx, (data, label) in enumerate(eval_loader):
            ...
            ssl_total_loss = ssl_total_loss + ssl_loss * len(label) + ssl_loss2 * len(label) + ssl_loss3 * len(label)
    ...
    label2_2 = torch.cat(ssl_gt2).cpu().detach().numpy()
    predicted2_2 = torch.cat(ssl_pred2).cpu().detach().numpy()
    acc_test2_2 = (predicted2_2 == label2_2).sum() * 100.0 / len(label2_2)
    label2_3 = torch.cat(ssl_gt3).cpu().detach().numpy()
    predicted2_3 = torch.cat(ssl_pred3).cpu().detach().numpy()
    acc_test2_3 = (predicted2_3 == label2_3).sum() * 100.0 / len(label2_3)
    label2 = torch.cat(ssl_gt).cpu().detach().numpy()
    predicted2 = torch.cat(ssl_pred).cpu().detach().numpy()
    ...
    if is_test:
        ...
        print('=> test acc: {:.2f}%, test F1: {:.4f} / ssl acc: {:.2f}%, ssl2 acc: {:.2f}%, ssl3 acc: {:.2f}%'.format(acc_test, f1_test, acc_test2, acc_test2_2, acc_test2_3))
        # logger.info('=> test acc: {:.2f}%, test F1: {:.4f} / ssl acc: {:.2f}%'.format(acc_test, f1_test, acc_test2))
        logger.info('=> test acc: {:.2f}%, test F1: {:.4f} / ssl acc: {:.2f}%, ssl2 acc: {:.2f}%, ssl3 acc: {:.2f}%'.format(acc_test, f1_test, acc_test2, acc_test2_2, acc_test2_3))
        c_mat = confusion_matrix(label, predicted)
